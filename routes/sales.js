...
router.post(
  ...
  [
    authenticator,
    [
      check("products", "Name is required")
        .not()
        .isEmpty()
    ]
  ],
  async (req, res) => {
    ...
    const { products, issuedTo } = req.body;

    try {
      {// let saleId = await Sale.findOne({ products });
        // let saleUser = await Sale.findOne({ user: req.user.id });
        // console.log(saleId, saleUser);
      ...
      for (let item of products) {
        ...
        if (productArr[0].amountAvailable < quantity) {
